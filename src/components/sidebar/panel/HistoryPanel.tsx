          ...
          history.map(item => (
            <div key={item.id} onClick={() => onNavigate(item.url)} className="p-3 bg-white/5 rounded-xl border border-white/5 shadow-sm hover:bg-white/10 transition-colors cursor-pointer group relative">
              <div className="flex items-center mb-1">
                {item.type === 'download' ? (
                  <Download size={14} className="text-blue-400 shrink-0 mr-3" />
                ) : (
                  <Globe size={14} className="text-white/30 shrink-0 mr-3" />
                )}
                <p className="text-sm text-white/90 truncate font-medium">
                  {item.title || item.url}
                </p>
              </div>
              <span className="text-[10px] text-white/40 truncate block pl-6">
                {item.url}
              </span>
            </div>
          ))
