    ...
    <div className="flex flex-col h-full w-full p-4 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pr-2 no-scrollbar space-y-2">
        {_.isEmpty(history) ? (
          ...
          history.map(item => (
            <div key={item.id} onClick={() => onNavigate(item.url)} className="p-3 bg-white/5 rounded-xl border border-white/5 shadow-sm hover:bg-white/10 transition-colors cursor-pointer group relative">
              <div className="flex items-center gap-2 mb-1">
                {item.type === 'download' ? (
                  ...
                  <svg className="w-3.5 h-3.5 text-white/30 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                )}
                <p className="text-sm text-white/90 truncate font-medium pr-2">
                  {item.title || item.url}
                </p>
              </div>
              <span className="text-[10px] text-white/40 truncate block pl-5.5">
                {item.url}
              </span>
            </div>
          ))
