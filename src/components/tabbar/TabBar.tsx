...
export const TabBar = ({ tabs, activeTabId, onTabClick, onCloseTab, onAddTab, onToggleSidebar }: TabBarProps) => {
  return (
    <div className="flex items-center flex-1 overflow-x-auto no-scrollbar">
      ...
      {tabs.map((tab) => (
        <div key={tab.id} onClick={() => onTabClick(tab.id)} className={`group relative flex items-center justify-between min-w-[120px] max-w-[180px] px-3 py-1 mr-1.5 rounded-md cursor-pointer transition-all duration-200 ${activeTabId === tab.id ? 'bg-white/15 text-white shadow-sm border border-white/10' : 'text-white/50 hover:bg-white/5 hover:text-white/80 border border-transparent'}`}>
